...
namespace Disassembler6510_v2
{
	class Program
	{
		...
		{
			args = new string[3]{"p01.prg", "0", "50" }; //debug
			int argc = args.Length + 1;
			string[] argv = new string[argc];
			for (int i = 0; i < args.Length; i++) { argv[i + 1] = args[i]; }
			int iCh;
			int[] iOpc = //new int[151]
			{
			...
			char ch;
			FileStream file_ptr = new FileStream(args[0], FileMode.Open);
			int iTest = 0;
			int iCount = 0;
			int iOperand16 = 0;
			int iOperand8 = 0;
				...
				if ((a > iNum2) && (a <= iNum2 + iNum3))
				{
					...
					if (iCount > 1)
					{
						...
						if (iCount == 2)
						{
							...
							if (iLength == 3) { printf("%5i %s %s ", __arglist(iOperand16, cDesc[iIndex], cMod[iIndex])); }
							if (iLength == 2) { printf("%5i %s %s ", __arglist( iOperand8, cDesc[iIndex], cMod[iIndex])); }
						}
						iCount--;
					}
					else
						...
						for (int i = 0; i <= 150; i++)
						{
							if (bNum1 == iOpc[i])
							{
								...
								if (iLines == 1)
								{
									...
									printf("\nadr   mne oprnd description                           mode ", __arglist());
								}
								printf("\n%05i ", __arglist(a - 1));
								iCount = iLen[i];
								iLength = iLen[i];
								printf("%s ", __arglist(cMne[i]));
								if (iLength == 1) { printf("      %s %s ", __arglist(cDesc[iIndex], cMod[iIndex])); }
							}
						}
						if (((iIndex == 0)) && ((bNum1 > 47) && (bNum1 < 58) || (bNum1 > 64) && (bNum1 < 91))) { printf("\n%05i data  '%c' ", __arglist(a - 1, ch)); }
						if (iIndex == 0) { printf("\n%05i byte %4i ", __arglist(a - 1, bNum1)); }
					}
			
				}
				// printf("%c", __arglist(ch));
			}
			if (iLines == 0)
			{
				...
				printf("dis 58360 14                  / disassemble some of the commodore 64c kernel\n", __arglist());//reset routine
			}
			file_ptr.Close();
			return;
		}

        private static unsafe void sscanf(string p, string v1, int* v2)
        {
			try { *v2 = int.Parse(p); } catch (Exception e) { *v2 = -1; }
        }
    }
}
